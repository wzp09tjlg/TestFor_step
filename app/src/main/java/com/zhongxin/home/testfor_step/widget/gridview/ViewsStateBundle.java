...
import static com.zhongxin.home.testfor_step.widget.gridview.BaseGridView.SAVE_ALL_CHILD;
import static com.zhongxin.home.testfor_step.widget.gridview.BaseGridView.SAVE_LIMITED_CHILD;
import static com.zhongxin.home.testfor_step.widget.gridview.BaseGridView.SAVE_NO_CHILD;
import static com.zhongxin.home.testfor_step.widget.gridview.BaseGridView.SAVE_ON_SCREEN_CHILD;

/**
 * Maintains a bundle of states for a group of views. Each view must have a
 * unique id to identify it. There are four different strategies
 * {@link BaseGridView#SAVE_NO_CHILD} {@link BaseGridView#SAVE_ON_SCREEN_CHILD}
 * {@link BaseGridView#SAVE_LIMITED_CHILD} {@link BaseGridView#SAVE_ALL_CHILD}.
 * <p>
 * This class serves purpose of nested "listview" e.g. a vertical list of
 * horizontal list. Vertical list maintains id->bundle mapping of all it's
 * children (even the children is offscreen and being pruned).
 * <p>
 * The class is currently used within {@link GridLayoutManager}, but it might be
 * used by other ViewGroup.
 */
class ViewsStateBundle
{
	...
	public final Bundle saveAsBundle()
	{
		...
		for (Iterator<Entry<String, SparseArray<Parcelable>>> i = snapshot
	...
	public final void loadFromBundle(Bundle savedBundle)
	{
		...
		{
			mChildStates.evictAll();
			for (Iterator<String> i = savedBundle.keySet().iterator(); i
			        .hasNext();)
			{
				String key = i.next();
				mChildStates
				        .put(key, savedBundle.getSparseParcelableArray(key));
			}
		}
	}
	 ...
	 *         {@link BaseGridView#SAVE_ALL_CHILD}
	 */
	...
	protected void applyPolicyChanges()
	{
		if (mSavePolicy == SAVE_LIMITED_CHILD)
		{
			...
			{
				mChildStates = new LruCache<String, SparseArray<Parcelable>>(
				        mLimitNumber);
			}
		}
		else if (mSavePolicy == SAVE_ALL_CHILD
		        || mSavePolicy == SAVE_ON_SCREEN_CHILD)
		{
	...
	/**
	 * Load view from states, it's none operation if the there is no state
	...
	public final void loadView(View view, int id)
	{
		if (mChildStates != null)
		{
			...
			// Once loaded the state, do not keep the state of child. The child
			// state will
			// be saved again either when child is offscreen or when the parent
			// is saved.
	...
	/**
	 * The on screen view is saved when policy is not
	 * {@link BaseGridView#SAVE_NO_CHILD}.
	 *
	 * @param bundle
	 *            Bundle where we save the on screen view state. If null, a new
	 *            Bundle is created and returned.
	 * @param view
	 *            The view to save.
	 * @param id
	 *            Id of the view.
