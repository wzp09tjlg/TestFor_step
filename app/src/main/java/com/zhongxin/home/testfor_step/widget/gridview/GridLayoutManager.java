 ...
 * the License.
 */
package com.zhongxin.home.testfor_step.widget.gridview;
...
final class GridLayoutManager extends RecyclerView.LayoutManager
{
	...
	/**
	 * Note on conventions in the presence of RTL layout directions: Many
	 * properties and method names reference entities related to the beginnings
	 * and ends of things. In the presence of RTL flows, it may not be clear
	 * whether this is intended to reference a quantity that changes direction
	 * in RTL cases, or a quantity that does not. Here are the conventions in
	 * use:
	 *
	 * start/end: coordinate quantities - do reverse (optical) left/right:
	 * coordinate quantities - do not reverse low/high: coordinate quantities -
	 * do not reverse min/max: coordinate quantities - do not reverse scroll
	 * offset - coordinate quantities - do not reverse first/last: positional
	 * indices - do not reverse front/end: positional indices - do not reverse
	 ...
	 * relationship does. In LTR flows, the first positional index is leftmost;
	 * in RTL flows, it is rightmost. Thus, anywhere that positional quantities
	 * are mapped onto coordinate quantities, the flow must be checked and the
	 * logic reversed.
	 */
	...
	private final Runnable mRequestLayoutRunnable = new Runnable()
	{
		...
		{
			if (DEBUG)
				Log.v(getTag(), "request Layout from runnable");
			requestLayout();
		}
	};
	/**
	...
	/**
	 * The offset to be applied to mFocusPosition, due to adapter change, on the
	 * next layout. Set to Integer.MIN_VALUE means we should stop adding delta
	 * to mFocusPosition until next layout cycler. TODO: This is somewhat
	 * duplication of RecyclerView getOldPosition() which is unfortunately
	 * cleared after prelayout.
	 */
	...
	/**
	 * Force a full layout under certain situations. E.g. Rows change, jump to
	 * invisible child.
	 */
	...
	/**
	 * The fixed size of each grid item in the secondary direction. This
	 * corresponds to the row height, equal for all rows. Grid items may have
	 * variable length in the primary direction.
	 */
	...
	/**
	 * Flag controlling whether the current/next layout should be updating the
	 * secondary size of rows.
	 */
	...
	/**
	 * Allow DPAD key to navigate out at the front of the View (where position =
	 * 0), default is false.
	 */
	...
	private Grid.Provider mGridProvider = new Grid.Provider()
	{
		...
		{
			if (TRACE)
				TraceHelper.beginSection("createItem");
			if (TRACE)
				TraceHelper.beginSection("getview");
			View v = getViewForPosition(index);
			if (TRACE)
				TraceHelper.endSection();
			LayoutParams lp = (LayoutParams) v.getLayoutParams();
			RecyclerView.ViewHolder vh = mBaseGridView.getChildViewHolder(v);
			lp.setItemAlignmentFacet(getFacet(vh, ItemAlignmentFacet.class));
			// See recyclerView docs: we don't need re-add scraped view if it
	        // was removed.
			if (!lp.isItemRemoved())
			{
				if (TRACE)
					TraceHelper.beginSection("addView");
				if (append)
				{
					addView(v);
				}
				...
				if (mPendingMoveSmoothScroller != null)
				{
					mPendingMoveSmoothScroller
	                        .consumePendingMovesBeforeLayout();
				}
				int subindex = getSubPositionByView(v, v.findFocus());
				if (!mInLayout)
				{
					// when we are appending item during scroll pass and the
	                // item's position
	                // matches the mFocusPosition, we should signal a
	                // childSelected event.
	                // However if we are still running
	                // PendingMoveSmoothScroller, we defer and
	                // signal the event in PendingMoveSmoothScroller.onStop().
	                // This can
	                // avoid lots of childSelected events during a long smooth
	                // scrolling and
				...
				{
					// fastRelayout will dispatch event at end of
	                // onLayoutChildren().
	                // For full layout, two situations here:
	                // 1. mInLayoutSearchFocus is false, dispatchChildSelected()
	                // at mFocusPosition.
	                // 2. mInLayoutSearchFocus is true: dispatchChildSelected()
	                // on first child
			...
			item[0] = v;
			return mOrientation == HORIZONTAL ? v.getMeasuredWidth()
	                : v.getMeasuredHeight();
		}

		@Override
		public void addItem(Object item, int index, int length, int rowIndex,
	            int edge)
		{
			View v = (View) item;
			int start, end;
			...
			{
				edge = !mGrid.isReversedFlow()
	                    ? mWindowAlignment.mainAxis().getPaddingLow()
			...
			{
				RecyclerView.ViewHolder vh = mBaseGridView
		...
		{
			if (TRACE)
				TraceHelper.beginSection("removeItem");
	...
	public void setOrientation(int orientation)
	{
		...
		{
			if (DEBUG)
				Log.v(getTag(), "invalid orientation: " + orientation);
			return;
		}
	...
	public void onRtlPropertiesChanged(int layoutDirection)
	{
		...
		mWindowAlignment.horizontal
		        .setReversedFlow(layoutDirection == View.LAYOUT_DIRECTION_RTL);
	}
	...
	public void setWindowAlignmentOffsetPercent(float offsetPercent)
	{
		mWindowAlignment.mainAxis()
		        .setWindowAlignmentOffsetPercent(offsetPercent);
	}
	...
	public void setItemAlignmentOffsetWithPadding(boolean withPadding)
	{
		mItemAlignment.mainAxis()
		        .setItemAlignmentOffsetWithPadding(withPadding);
		updateChildAlignments();
	}
	...
	private int getSubPositionByView(View view, View childView)
	{
		...
		if (facet != null)
		{
			final ItemAlignmentFacet.ItemAlignmentDef[] defs = facet
			        .getAlignmentDefs();
			if (defs.length > 1)
			{
	...
	private void dispatchChildSelected()
	{
		if (mChildSelectedListener == null
		        && mChildViewHolderSelectedListener == null)
		{
			return;
		}

		if (TRACE)
			TraceHelper.beginSection("onChildSelected");
		View view = mFocusPosition == NO_POSITION ? null
		        : findViewByPosition(mFocusPosition);
		if (view != null)
		{
			RecyclerView.ViewHolder vh = mBaseGridView.getChildViewHolder(view);
			if (mChildSelectedListener != null)
			{
				mChildSelectedListener.onChildSelected(mBaseGridView, view,
				        mFocusPosition, vh == null ? NO_ID : vh.getItemId());
			}
			if (mChildViewHolderSelectedListener != null)
			{
				mChildViewHolderSelectedListener.onChildViewHolderSelected(
				        mBaseGridView, vh, mFocusPosition, mSubFocusPosition);
			}
		}
		else
		{
			if (mChildSelectedListener != null)
			{
				mChildSelectedListener.onChildSelected(mBaseGridView, null,
				        NO_POSITION, NO_ID);
			}
			if (mChildViewHolderSelectedListener != null)
			{
				mChildViewHolderSelectedListener.onChildViewHolderSelected(
				        mBaseGridView, null, NO_POSITION, 0);
			}
		}
		if (TRACE)
			TraceHelper.endSection();

		// Children may request layout when a child selection event occurs (such
		// as a change of
		// padding on the current and previously selected rows).
		// If in layout, a child requesting layout may have been laid out before
		// the selection
		// callback.
		// If it was not, the child will be laid out after the selection
		// callback.
		// If so, the layout request will be honoured though the view system
		// will emit a double-
		// layout warning.
		// If not in layout, we may be scrolling in which case the child layout
		// request will be
	...
	@Override
	public RecyclerView.LayoutParams generateLayoutParams(Context context,
	        AttributeSet attrs)
	{
	...
	{
		return (mOrientation == HORIZONTAL) ? getOpticalLeft(v)
		        : getOpticalTop(v);
	}

	private int getViewMax(View v)
	{
		return (mOrientation == HORIZONTAL) ? getOpticalRight(v)
		        : getOpticalBottom(v);
	}
	...
	private int getViewPrimarySize(View view)
	{
		...
		return mOrientation == HORIZONTAL ? p.getOpticalWidth(view)
		        : p.getOpticalHeight(view);
	}

	private int getViewCenter(View view)
	{
		return (mOrientation == HORIZONTAL) ? getViewCenterX(view)
		        : getViewCenterY(view);
	}

	private int getViewCenterSecondary(View view)
	{
		return (mOrientation == HORIZONTAL) ? getViewCenterY(view)
		        : getViewCenterX(view);
	}
	...
	/**
	 * Save Recycler and State for convenience. Must be paired with
	 * leaveContext().
	 */
	...
	private void leaveContext()
	{
		// 如果是翻页效果就不释放
		if (mBaseGridView
		        .getFocusScrollStrategy() != BaseGridView.FOCUS_SCROLL_PAGE)
		{
			mRecycler = null;
			mState = null;
		}
	...
	private boolean layoutInit()
	{
		...
		else
		{
			...
			int firstVisibleIndex = focusViewWasInTree
			        ? mGrid.getFirstVisibleIndex() : 0;
			...
			{
				// if focusView was in tree, we will add item from first visible
				// item
				mGrid.setStart(firstVisibleIndex);
			}
			else
			{
				// if focusView was not in tree, it's probably because focus
				// position jumped
	...
	private int getSizeSecondary()
	{
		...
		return getRowStartSecondary(rightmostIndex)
		        + getRowSizeSecondary(rightmostIndex);
	}
	...
	private boolean processRowSizeSecondary(boolean measure)
	{
		...
		CircularIntArray[] rows = mGrid == null ? null
		        : mGrid.getItemPositionsInRows();
		...
		for (int rowIndex = 0; rowIndex < mNumRows; rowIndex++)
		{
			...
			if (measure && rowSize < 0 && itemCount > 0)
			{
				...
				rowSize = mOrientation == HORIZONTAL ? scrapChildHeight
				        : scrapChildWidth;
			}
			if (rowSize < 0)
			{
			...
			{
				if (DEBUG)
					Log.v(getTag(), "row size secondary changed: "
	        ...
	        int heightSpec)
	{
		...
		if (DEBUG)
			Log.v(getTag(),
                    "onMeasure widthSpec " + Integer.toHexString(widthSpec)
                            + " heightSpec " + Integer.toHexString(heightSpec)
                            + " modeSecondary "
                            + Integer.toHexString(modeSecondary)
		...
		if (mRowSizeSecondaryRequested == ViewGroup.LayoutParams.WRAP_CONTENT)
		{
			...
			if (mRowSizeSecondary == null
			        || mRowSizeSecondary.length != mNumRows)
			{
			...
			switch (modeSecondary)
			{
				case MeasureSpec.UNSPECIFIED:
					measuredSizeSecondary = getSizeSecondary()
					        + paddingSecondary;
					break;
				case MeasureSpec.AT_MOST:
					measuredSizeSecondary = Math.min(
                            getSizeSecondary() + paddingSecondary,
                            mMaxSizeSecondary);
					break;
					...
					if (mRowSizeSecondaryRequested == 0)
					{
						if (mOrientation == HORIZONTAL)
						{
							throw new IllegalStateException(
							        "Must specify rowHeight or view height");
						}
						else
						{
							throw new IllegalStateException(
							        "Must specify columnWidth or view width");
						}
					}
					mFixedRowSizeSecondary = mRowSizeSecondaryRequested;
					mNumRows = mNumRowsRequested == 0 ? 1 : mNumRowsRequested;
					measuredSizeSecondary = mFixedRowSizeSecondary * mNumRows
					        + mMarginSecondary * (mNumRows - 1)
					        + paddingSecondary;
					break;
				case MeasureSpec.AT_MOST:
				case MeasureSpec.EXACTLY:
					if (mNumRowsRequested == 0
					        && mRowSizeSecondaryRequested == 0)
					{
						mNumRows = 1;
						mFixedRowSizeSecondary = sizeSecondary
						        - paddingSecondary;
					}
					...
					else if (mRowSizeSecondaryRequested == 0)
					{
						...
						mFixedRowSizeSecondary = (sizeSecondary
						        - paddingSecondary
						        - mMarginSecondary * (mNumRows - 1)) / mNumRows;
					}
					else
					{
						mNumRows = mNumRowsRequested;
					...
					{
						int childrenSize = mFixedRowSizeSecondary * mNumRows
						        + mMarginSecondary * (mNumRows - 1)
						        + paddingSecondary;
		...
		{
			Log.v(getTag(),
                    "onMeasure sizePrimary " + sizePrimary
                            + " measuredSizeSecondary " + measuredSizeSecondary
                            + " mFixedRowSizeSecondary "
                            + mFixedRowSizeSecondary + " mNumRows " + mNumRows);
		}
		leaveContext();
	}
	...
	private void measureChild(View child)
	{
		...
		final int secondarySpec = (mRowSizeSecondaryRequested == ViewGroup.LayoutParams.WRAP_CONTENT)
		        ...
		        : MeasureSpec.makeMeasureSpec(mFixedRowSizeSecondary,
                MeasureSpec.EXACTLY);
		...
		if (mOrientation == HORIZONTAL)
		{
			widthSpec = ViewGroup.getChildMeasureSpec(
                    ...
                    lp.width);
			heightSpec = ViewGroup.getChildMeasureSpec(secondarySpec, 0,
                    lp.height);
		}
		else
		{
			heightSpec = ViewGroup.getChildMeasureSpec(
                    MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED), 0,
                    lp.height);
			widthSpec = ViewGroup.getChildMeasureSpec(secondarySpec, 0,
                    lp.width);
		}
		child.measure(widthSpec, heightSpec);
		if (DEBUG)
			Log.v(getTag(),
                    "measureChild secondarySpec "
                            + Integer.toHexString(secondarySpec) + " widthSpec "
                            + Integer.toHexString(widthSpec) + " heightSpec "
                            + Integer.toHexString(heightSpec)
                            + " measuredWidth " + child.getMeasuredWidth()
                            + " measuredHeight " + child.getMeasuredHeight());
		if (DEBUG)
			Log.v(getTag(),
                    "child lp width " + lp.width + " height " + lp.height);
		if (TRACE)
			TraceHelper.endSection();
	}
	 ...
	 */
	private <E> E getFacet(RecyclerView.ViewHolder vh,
	        Class<? extends E> facetClass)
	{
		E facet = null;
		...
		{
			FacetProvider p = mFacetProviderAdapter
			        .getFacetProvider(vh.getItemViewType());
			if (p != null)
			{
	...
	private void layoutChild(int rowIndex, View v, int start, int end,
	        int startSecondary)
	{
		if (TRACE)
		...
		final int horizontalGravity = (mReverseFlowPrimary
		        || mReverseFlowSecondary)
	...
	private void updateChildOpticalInsets(View v, int left, int top, int right,
	        int bottom)
	{
		LayoutParams p = (LayoutParams) v.getLayoutParams();
	...
	/**
	 * Fast layout when there is no structure change, adapter change, etc. It
	 * will layout all views was layout requested or updated, until hit a view
	 * with different size, then it break and detachAndScrap all views after
	 * that.
	 */
	private void fastRelayout()
	{
		...
		for (int index = 0; index < childCount; index++)
		{
			...
			if (location == null)
			{
				if (DEBUG)
					Log.w(getTag(),
                            "fastRelayout(): no Location at " + position);
			...
			if (oldPrimarySize != primarySize)
			{
				...
				if (DEBUG)
					Log.d(getTag(),
                            "fastRelayout: view size changed at " + position);
				invalidateAfter = true;
				break;
			}
		}
		if (invalidateAfter)
		{
			...
			if (mPruneChild)
			{
				// in regular prune child mode, we just append items up to edge
				// limit
				appendVisibleItems();
				if (mFocusPosition >= 0 && mFocusPosition <= savedLastPos)
				{
					// make sure add focus view back: the view might be outside
					// edge limit
			...
			{
				// prune disabled(e.g. in RowsFragment transition): append all
				// removed items
	...
	{
		if (TRACE)
			TraceHelper.beginSection("removeAndRecycleAllViews");
		if (DEBUG)
			Log.v(TAG, "removeAndRecycleAllViews " + getChildCount());
	...
	public void onLayoutChildren(Recycler recycler, State state)
	{
		if (DEBUG)
		{
			Log.v(getTag(), "layoutChildren start numRows " + mNumRows
                    + " mScrollOffsetSecondary " + mScrollOffsetSecondary
		...
		// We must use same delta in Pre Layout (if prelayout exists) and second
		// layout.
		// So we cache the deltas in PreLayout and use it in second layout.
		int delta = 0, deltaSecondary = 0;
		if (mFocusPosition != NO_POSITION && scrollToFocus && mBaseGridView
		        .getScrollState() != RecyclerView.SCROLL_STATE_IDLE)
		{
			// FIXME: we should get the remaining scroll animation offset from
			// RecyclerView
		...
		else
		{
			...
			// multiple rounds: scrollToView of first round may drag first/last
			// child into
			// "visible window" and we update scrollMin/scrollMax then run
			// second scrollToView
			int oldFirstVisible;
			int oldLastVisible;
			do
		...
		{
			// For full layout we dispatchChildSelected() in createItem() unless
			// searched all
			// children and found none is focusable then dispatchChildSelected()
			// here.
			dispatchChildSelected();
		}
	...
	{
		if (DEBUG)
			Log.v(getTag(), "scrollVerticallyBy " + dy);
		if (!mLayoutEnabled || !hasDoneFirstLayout())
		{
			return 0;
		}
	...
	{
		if (TRACE)
			TraceHelper.beginSection("scrollPrimary");
		boolean isMaxUnknown, isMinUnknown;
		...
		if (TRACE)
			TraceHelper.beginSection("remove");
		if (mReverseFlowPrimary ? da > 0 : da < 0)
		{
			removeInvisibleViewsAtEnd();
		}
	...
	{
		int highVisiblePos = (!mReverseFlowPrimary)
		        ? mGrid.getLastVisibleIndex() : mGrid.getFirstVisibleIndex();
	...
	{
		int lowVisiblePos = (!mReverseFlowPrimary)
		        ? mGrid.getFirstVisibleIndex() : mGrid.getLastVisibleIndex();
	...
	private void initScrollController()
	{
		...
		mWindowAlignment.horizontal.setPadding(getPaddingLeft(),
		        getPaddingRight());
		mWindowAlignment.vertical.setPadding(getPaddingTop(),
		        getPaddingBottom());
		...
		{
			Log.v(getTag(),
                    "initScrollController mSizePrimary " + mSizePrimary
	...
	private void updateScrollController()
	{
		...
		// mScrollOffsetSecondary is -16. fastRelayout() put views based on
		// offsets(not padding),
		// when padding changes to 20, we also need update
		// mScrollOffsetSecondary to -20 before
		// fastRelayout() is performed
		int paddingPrimaryDiff, paddingSecondaryDiff;
		if (mOrientation == HORIZONTAL)
		{
			paddingPrimaryDiff = getPaddingLeft()
			        - mWindowAlignment.horizontal.getPaddingLow();
			paddingSecondaryDiff = getPaddingTop()
			        - mWindowAlignment.vertical.getPaddingLow();
		}
		else
		{
			paddingPrimaryDiff = getPaddingTop()
			        - mWindowAlignment.vertical.getPaddingLow();
			paddingSecondaryDiff = getPaddingLeft()
			        - mWindowAlignment.horizontal.getPaddingLow();
		}
		...
		{
			Log.v(getTag(),
                    "updateScrollController mSizePrimary " + mSizePrimary
                            + " mWindowAlignment " + mWindowAlignment
                            + " mScrollOffsetPrimary " + mScrollOffsetPrimary);
		}
	}
	...
	public void setSelectionWithSub(RecyclerView parent, int position,
	        int subposition, int primaryScrollExtra)
	{
		setSelection(parent, position, subposition, false, primaryScrollExtra);
	}

	public void setSelectionSmoothWithSub(RecyclerView parent, int position,
	        int subposition)
	{
	...
	public void setSelection(RecyclerView parent, int position, int subposition,
	        boolean smooth, int primaryScrollExtra)
	{
		if (mFocusPosition != position && position != NO_POSITION
		        || subposition != mSubFocusPosition
		        || primaryScrollExtra != mPrimaryScrollExtra)
		{
			scrollToSelection(parent, position, subposition, smooth,
			        primaryScrollExtra);
		}
	}

	private void scrollToSelection(RecyclerView parent, int position,
	        int subposition, boolean smooth, int primaryScrollExtra)
	{
		if (TRACE)
			TraceHelper.beginSection("scrollToSelection");
	...
	void startPositionSmoothScroller(int position)
	{
		LinearSmoothScroller linearSmoothScroller = new GridLinearSmoothScroller()
		{
			...
			public PointF computeScrollVectorForPosition(int targetPosition)
			{
				...
				// TODO We should be able to deduce direction from bounds of
		        // current and target
		        // focus, rather than making assumptions about positions and
		        // directionality
				final boolean isStart = mReverseFlowPrimary
	...
	private void processPendingMovement(boolean forward)
	{
		...
		if (mPendingMoveSmoothScroller == null)
		{
			// Stop existing scroller and create a new
	...
	@Override
	public void onItemsAdded(RecyclerView recyclerView, int positionStart,
	        int itemCount)
	{
		if (DEBUG)
			Log.v(getTag(), "onItemsAdded positionStart " + positionStart
                    + " itemCount " + itemCount);
		if (mFocusPosition != NO_POSITION
		        && mFocusPositionOffset != Integer.MIN_VALUE)
		{
	...
	@Override
	public void onItemsRemoved(RecyclerView recyclerView, int positionStart,
	        int itemCount)
	{
		if (DEBUG)
			Log.v(getTag(), "onItemsRemoved positionStart " + positionStart
                    + " itemCount " + itemCount);
		if (mFocusPosition != NO_POSITION
		        && mFocusPositionOffset != Integer.MIN_VALUE)
		{
	...
	@Override
	public void onItemsMoved(RecyclerView recyclerView, int fromPosition,
	        int toPosition, int itemCount)
	{
		if (DEBUG)
			Log.v(getTag(), "onItemsMoved fromPosition " + fromPosition
	...
	{
		if (DEBUG)
			Log.v(getTag(), "onItemsUpdated positionStart " + positionStart
                    + " itemCount " + itemCount);
		for (int i = positionStart, end = positionStart
		        + itemCount; i < end; i++)
		{
	...
	@Override
	public boolean onRequestChildFocus(RecyclerView parent, View child,
	        View focused)
	{
	...
	@Override
	public boolean requestChildRectangleOnScreen(RecyclerView parent, View view,
	        Rect rect, boolean immediate)
	{
		if (DEBUG)
			Log.v(getTag(),
                    "requestChildRectangleOnScreen " + view + " " + rect);
		return false;
	}

	int getScrollOffsetX()
	{
		return mOrientation == HORIZONTAL ? mScrollOffsetPrimary
		        : mScrollOffsetSecondary;
	}

	int getScrollOffsetY()
	{
		return mOrientation == HORIZONTAL ? mScrollOffsetSecondary
	...
	public void getViewSelectedOffsets(View view, int[] offsets)
	{
		if (mOrientation == HORIZONTAL)
		{
			offsets[0] = getPrimarySystemScrollPosition(view)
			        - mScrollOffsetPrimary;
			offsets[1] = getSecondarySystemScrollPosition(view)
			        - mScrollOffsetSecondary;
		}
		else
		{
			offsets[1] = getPrimarySystemScrollPosition(view)
			        - mScrollOffsetPrimary;
			offsets[0] = getSecondarySystemScrollPosition(view)
			        - mScrollOffsetSecondary;
		}
	}
	...
	{
		final int viewCenterPrimary = mScrollOffsetPrimary
		        + getViewCenter(view);
		...
		return mWindowAlignment.mainAxis().getSystemScrollPos(viewCenterPrimary,
		        isMin, isMax);
	}

	/**
	 * Get adjusted primary position for a given childView (if there is multiple
	 * ItemAlignment defined on the view).
	 */
	private int getAdjustedPrimaryScrollPosition(int scrollPrimary, View view,
	        View childView)
	{
		...
		{
			final LayoutParams lp = (LayoutParams) view.getLayoutParams();
			scrollPrimary += lp.getAlignMultiple()[subindex]
	...
	private int getSecondarySystemScrollPosition(View view)
	{
		...
		return mWindowAlignment.secondAxis()
		        .getSystemScrollPos(viewCenterSecondary, isMin, isMax);
	}
	...
	private void scrollToView(View view, View childView, boolean smooth)
	{
		...
		if (newFocusPosition != mFocusPosition
		        || newSubFocusPosition != mSubFocusPosition)
		{
		...
		{
			// transfer focus to the child if it does not have focus yet (e.g.
			// triggered
			// by setSelection())
			view.requestFocus();
		}
	...
	private boolean getNoneAlignedPosition(View view, int[] deltas)
	{
		...
		if (viewMin < paddingLow)
		{
			...
			if (mFocusScrollStrategy == BaseGridView.FOCUS_SCROLL_PAGE)
			{
				...
				// align first visible item of the "page" at the low padding
				// edge.
				while (prependOneColumnVisibleItems())
				{
					CircularIntArray positions = mGrid.getItemPositionsInRows(
					        mGrid.getFirstVisibleIndex(), pos)[row];
		...
		else if (viewMax > clientSize + paddingLow)
		{
			...
			if (mFocusScrollStrategy == BaseGridView.FOCUS_SCROLL_PAGE)
			{
				// scroll whole one page right/bottom, align view at the low
				// padding edge.
				firstView = view;
				do
				{
					CircularIntArray positions = mGrid.getItemPositionsInRows(
					        pos, mGrid.getLastVisibleIndex())[row];
					lastView = findViewByPosition(
		...
		scrollSecondary = getSecondarySystemScrollPosition(
	...
	private boolean getAlignedPosition(View view, View childView, int[] deltas)
	{
		...
		if (childView != null)
		{
			scrollPrimary = getAdjustedPrimaryScrollPosition(scrollPrimary,
			        view, childView);
		}
		...
		{
			Log.v(getTag(),
                    "getAlignedPosition " + scrollPrimary + " "
                            + scrollSecondary + " " + mPrimaryScrollExtra + " "
                            + mWindowAlignment);
	...
	private void scrollGrid(int scrollPrimary, int scrollSecondary,
	        boolean smooth)
	{
		if (mInLayout)
		{
	...
	public void setScrollEnabled(boolean scrollEnabled)
	{
		if (mScrollEnabled != scrollEnabled)
		{
			...
			if (mScrollEnabled
			        ...
			        && mFocusPosition != NO_POSITION)
			{
				scrollToSelection(mBaseGridView, mFocusPosition,
				        mSubFocusPosition, true, mPrimaryScrollExtra);
			}
		}
	}
	        ...
	        ArrayList<View> views, int direction, int focusableMode)
	{
		...
		// consider our children for focus searching in main direction on the
		// same row.
		// If this viewgroup has no focus and using focus align, we want the
		// system
		// to ignore our children and pass focus to the viewgroup, which will
		// pass
		...
		if (recyclerView.hasFocus())
		{
			...
			final int focusedPos = getPositionByIndex(
			        findImmediateChildIndex(focused));
			// Add focusables of focused item.
			if (focusedPos != NO_POSITION)
			{
				findViewByPosition(focusedPos).addFocusables(views, direction,
				        focusableMode);
			}
			final int focusedRow = mGrid != null && focusedPos != NO_POSITION
			        ? mGrid.getLocation(focusedPos).row : NO_POSITION;
			// Add focusables of next neighbor of same row on the focus search
			// direction.
			if (mGrid != null)
			{
		...
		else
		{
			...
			{
				// adding views not overlapping padding area to avoid scrolling
				// in gaining focus
				...
				for (int i = 0, count = getChildCount(); i < count; i++)
				{
					...
					if (child.getVisibility() == View.VISIBLE)
					{
						if (getViewMin(child) >= left
						        && getViewMax(child) <= right)
						{
							child.addFocusables(views, direction,
							        focusableMode);
						}
	...
	{
		if (DEBUG)
			Log.v(getTag(), "onFocusSearchFailed direction " + direction);

		View view = null;
		int movement = getMovement(direction);
		final boolean isScroll = mBaseGridView
		        .getScrollState() != RecyclerView.SCROLL_STATE_IDLE;
		saveContext(recycler, state);
		...
		leaveContext();
		if (DEBUG)
			Log.v(getTag(), "onFocusSearchFailed returning view " + view);
		return view;
	}

	boolean gridOnRequestFocusInDescendants(RecyclerView recyclerView,
	...
	private boolean gridOnRequestFocusInDescendantsAligned(
	        RecyclerView recyclerView, int direction,
	        Rect previouslyFocusedRect)
	{
		View view = findViewByPosition(mFocusPosition);
		if (view != null)
		{
			boolean result = view.requestFocus(direction,
	...
	private boolean gridOnRequestFocusInDescendantsUnaligned(
	        RecyclerView recyclerView, int direction,
	        Rect previouslyFocusedRect)
	{
		// focus to view not overlapping padding area to avoid scrolling in
		// gaining focus
		int index;
		int increment;
		int end;
	...
	{
		if (DEBUG)
			Log.v(getTag(), "onAdapterChanged to " + newAdapter);
		if (oldAdapter != null)
		{
			discardLayoutInfo();
			mFocusPosition = NO_POSITION;
	...
	public Parcelable onSaveInstanceState()
	{
		if (DEBUG)
			Log.v(getTag(),
                    "onSaveInstanceState getSelection() " + getSelection());
		...
		for (int i = 0, count = getChildCount(); i < count; i++)
		{
			...
			{
				bundle = mChildrenStates.saveOnScreenView(bundle, view,
				        position);
			}
		}
	...
	public void onRestoreInstanceState(Parcelable state)
	{
		...
		if (DEBUG)
			Log.v(getTag(),
                    "onRestoreInstanceState mFocusPosition " + mFocusPosition);
	}
	...
	private int computeScrollOffset(State state)
	{
		...
		else
		{
			...
			return ScrollbarHelper.computeScrollOffset(getItemCount(),
			        this.mOrientationHelper,
			        this.findFirstVisibleChildClosestToStart(
			                !this.mSmoothScrollbarEnabled, true),
			        this.findFirstVisibleChildClosestToEnd(
			                !this.mSmoothScrollbarEnabled, true),
	...
	private int computeScrollExtent(State state)
	{
		...
		{
			this.ensureLayoutState();
			return ScrollbarHelper.computeScrollExtent(getItemCount(),
			        this.mOrientationHelper,
			        this.findFirstVisibleChildClosestToStart(
			                !this.mSmoothScrollbarEnabled, true),
			        this.findFirstVisibleChildClosestToEnd(
	...
	private int computeScrollRange(State state)
	{
		...
		{
			this.ensureLayoutState();
			return ScrollbarHelper.computeScrollRange(getItemCount(),
			        this.mOrientationHelper,
			        this.findFirstVisibleChildClosestToStart(
			                !this.mSmoothScrollbarEnabled, true),
			        this.findFirstVisibleChildClosestToEnd(
	...
	void ensureLayoutState()
	{
		if (this.mOrientationHelper == null)
		{
			this.mOrientationHelper = OrientationHelper
			        .createOrientationHelper(this, this.mOrientation);
		}
	}
	...
	{
		return this.findOneVisibleChild(0, this.getChildCount(),
		        completelyVisible, acceptPartiallyVisible);
	}

	private View findFirstVisibleChildClosestToEnd(boolean completelyVisible,
	        boolean acceptPartiallyVisible)
	{
		return this.findOneVisibleChild(this.getChildCount() - 1, -1,
		        completelyVisible, acceptPartiallyVisible);
	}
	...
	{
		return mGrid.getFirstVisibleIndex();
	}
	...
	{
		return mGrid.getLastVisibleIndex();
	}

	/*
	 * LayoutParams for {@link HorizontalGridView} and {@link VerticalGridView}.
	 * The class currently does two internal jobs: - Saves optical bounds
	 * insets. - Caches focus align view center.
	 */
	static class LayoutParams extends RecyclerView.LayoutParams
	{
		...
		{
			ItemAlignmentFacet.ItemAlignmentDef[] defs = mAlignmentFacet
			        .getAlignmentDefs();
		...
		void setOpticalInsets(int leftInset, int topInset, int rightInset,
		        int bottomInset)
		{
	...
	final static class SavedState implements Parcelable
	{
		...
		SavedState(Parcel in)
		{
			index = in.readInt();
			childStates = in
			        .readBundle(GridLayoutManager.class.getClassLoader());
		}
	...
	abstract class GridLinearSmoothScroller extends LinearSmoothScroller
	{
		...
		@Override
		protected void onTargetFound(View targetView, State state,
		        Action action)
		{
	...
	/**
	 * The SmoothScroller that remembers pending DPAD keys and consume pending
	 * keys during scroll.
	 */
	final class PendingMoveSmoothScroller extends GridLinearSmoothScroller
	{
		// -2 is a target position that LinearSmoothScroller can never find
		// until
		...
		// Number of pending movements on primary direction, negative if
		// PREV_ITEM.
		private int mPendingMoves;

		PendingMoveSmoothScroller(int initialPendingMoves,
		        boolean staggeredGrid)
		{
		...
		/**
		 * Called before laid out an item when non-staggered grid can handle
		 * pending movements by skipping "mNumRows" per movement; staggered grid
		 * will have to wait the item has been laid out in
		...
		void consumePendingMovesBeforeLayout()
		{
			...
			for (int pos = startPos; mPendingMoves != 0; pos = mPendingMoves > 0
			        ? pos + mNumRows : pos - mNumRows)
			{
		...
		/**
		 * Called after laid out an item. Staggered grid should find view on
		 * same Row and consume pending movements.
		 */
		void consumePendingMovesAfterLayout()
		{
			if (mStaggeredGrid && mPendingMoves != 0)
			{
				...
				final int focusedRow = mGrid != null
				        && mFocusPosition != NO_POSITION
				                ? mGrid.getLocation(mFocusPosition).row
				                : NO_POSITION;
				View newSelected = null;
				for (int i = 0, count = getChildCount(); i < count
				        && mPendingMoves != 0; i++)
				{
			...
			if (mPendingMoves == 0
			        || (mPendingMoves > 0 && hasCreatedLastItem())
		...
		public PointF computeScrollVectorForPosition(int targetPosition)
		{
			...
			int direction = (mReverseFlowPrimary ? mPendingMoves > 0
			        : mPendingMoves < 0) ? -1 : 1;
			if (mOrientation == HORIZONTAL)
			{
				return new PointF(direction, 0);
