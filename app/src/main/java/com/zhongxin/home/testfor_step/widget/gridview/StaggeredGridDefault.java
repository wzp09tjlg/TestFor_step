...
final class StaggeredGridDefault extends StaggeredGrid
{
	...
	/**
	 * Note this method has assumption that item is filled either in the same
	 * row next row of last item. Search until row index wrapped.
	 */
	@Override
	public int findRowMax(boolean findLarge, int indexLimit, int[] indices)
	{
		...
		{
			value = edge;
			for (int i = indexLimit + 1; visitedRows < mNumRows
			        && i <= mLastVisibleIndex; i++)
			{
				loc = getLocation(i);
				edge += loc.offset;
				if (loc.row != visitRow)
	...
	@Override
	protected boolean appendVisibleItemsWithoutCache(int toLimit,
	        boolean oneColumnMode)
	{
		...
		if (mLastVisibleIndex >= 0)
		{
			...
			{
				edgeLimit = mReversedFlow ? findRowMin(false,
				        edgeLimitSearchIndex, null) : findRowMax(true,
				        edgeLimitSearchIndex, null);
			}
			        ...
			        : getRowMax(rowIndex) >= edgeLimit)
			{
				...
				if (rowIndex == mNumRows)
				{
					...
					rowIndex = 0;
					edgeLimit = mReversedFlow ? findRowMin(false, null)
					        : findRowMax(true, null);
				}
			}
		...
		else
		{
			...
			rowIndex = (mLocations.size() > 0 ? getLocation(getLastIndex()).row + 1
			        : itemIndex)
			        % mNumRows;
			edgeLimit = 0;
		...
		while (true)
		{
			// find end-most row edge (.high is biggest, or .low is smallest in
			// reversed flow)
			// fill from current row till last row so that each row will grow
			// longer than
			...
			for (; rowIndex < mNumRows; rowIndex++)
			{
				...
				if (itemIndex == count
				        || (!oneColumnMode && checkAppendOverLimit(toLimit)))
				{
					return filledOne;
				}
				int location = mReversedFlow ? getRowMin(rowIndex)
				        : getRowMax(rowIndex);
				        ...
				        || location == Integer.MIN_VALUE)
				{
					...
					if (rowIndex == 0)
					{
						location = mReversedFlow ? getRowMin(mNumRows - 1)
						        : getRowMax(mNumRows - 1);
						if (location != Integer.MAX_VALUE
						        && location != Integer.MIN_VALUE)
						{
							location = location
							        + (mReversedFlow ? -mMargin : mMargin);
						}
					}
					else
					{
						location = mReversedFlow ? getRowMax(rowIndex - 1)
						        : getRowMin(rowIndex - 1);
					}
				}
				else
				{
				...
				int size = appendVisibleItemToRow(itemIndex++, rowIndex,
				        location);
				filledOne = true;
				// fill more item to the row to make sure this row is longer
				// than
				// the previous highest row.
				if (edgeLimitIsValid)
				{
					        ...
					        : location + size < edgeLimit)
					{
						...
						location = location
						        + (mReversedFlow ? -size - mMargin : size
						                + mMargin);
						size = appendVisibleItemToRow(itemIndex++, rowIndex,
						        location);
					}
				}
				else
				{
					edgeLimitIsValid = true;
					edgeLimit = mReversedFlow ? getRowMin(rowIndex)
					        : getRowMax(rowIndex);
				}
			}
			if (oneColumnMode)
			...
			edgeLimit = mReversedFlow ? findRowMin(false, null) : findRowMax(
			        true, null);
	...
	@Override
	protected boolean prependVisibleItemsWithoutCache(int toLimit,
	        boolean oneColumnMode)
	{
		int itemIndex;
		int rowIndex;
		...
		if (mFirstVisibleIndex >= 0)
		{
			...
			{
				edgeLimit = mReversedFlow ? findRowMax(true,
				        edgeLimitSearchIndex, null) : findRowMin(false,
				        edgeLimitSearchIndex, null);
			}
		...
		{
			itemIndex = mStartIndex != START_DEFAULT ? mStartIndex : 0;
			// if there are cached items, put on previous row of first cached
			// item.
			rowIndex = (mLocations.size() >= 0 ? getLocation(getFirstIndex()).row
			        + mNumRows - 1
			        : itemIndex)
			        % mNumRows;
			edgeLimit = 0;
		...
		while (true)
		{
			// find start-most row edge (.low is smallest, or .high is largest
			// in reversed flow)
			// fill from current row till first row so that each row will grow
			// longer than
			...
			for (; rowIndex >= 0; rowIndex--)
			{
				// fill one item to a row
				if (itemIndex < 0
				        || (!oneColumnMode && checkPrependOverLimit(toLimit)))
				{
					return filledOne;
				}
				int location = mReversedFlow ? getRowMax(rowIndex)
				        : getRowMin(rowIndex);
				if (location == Integer.MAX_VALUE
				        || location == Integer.MIN_VALUE)
				{
					...
					{
						location = mReversedFlow ? getRowMin(rowIndex + 1)
						        : getRowMax(rowIndex + 1);
					}
				}
				else
				{
				...
				int size = prependVisibleItemToRow(itemIndex--, rowIndex,
				        location);
				filledOne = true;
				...
				if (edgeLimitIsValid)
				{
					        ...
					        : location - size > edgeLimit)
					{
						...
						location = location
						        + (mReversedFlow ? size + mMargin : -size
						                - mMargin);
						size = prependVisibleItemToRow(itemIndex--, rowIndex,
						        location);
					}
				}
				else
				{
					edgeLimitIsValid = true;
					edgeLimit = mReversedFlow ? getRowMax(rowIndex)
					        : getRowMin(rowIndex);
				}
			}
			if (oneColumnMode)
			...
			edgeLimit = mReversedFlow ? findRowMax(true, null) : findRowMin(
			        false, null);
