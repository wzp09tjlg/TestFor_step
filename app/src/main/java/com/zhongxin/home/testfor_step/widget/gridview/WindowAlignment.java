...
import static com.zhongxin.home.testfor_step.widget.gridview.BaseGridView.WINDOW_ALIGN_BOTH_EDGE;
import static com.zhongxin.home.testfor_step.widget.gridview.BaseGridView.WINDOW_ALIGN_HIGH_EDGE;
import static com.zhongxin.home.testfor_step.widget.gridview.BaseGridView.WINDOW_ALIGN_LOW_EDGE;
import static com.zhongxin.home.testfor_step.widget.gridview.BaseGridView.WINDOW_ALIGN_OFFSET_PERCENT_DISABLED;
...
class WindowAlignment
{
	...
	{
		return "horizontal=" + horizontal.toString() + "; vertical=" + vertical.toString();
	}
	...
	{
		/**
		 * mScrollCenter is used to calculate dynamic transformation based on
		 * how far a view is from the mScrollCenter. For example, the views with
		 * center close to mScrollCenter will be scaled up.
		 */
		...
		final public int getSystemScrollPos(int scrollCenter, boolean isAtMin,
		        boolean isAtMax)
		{
			int middlePosition;
			if (!mReversedFlow)
			{
				...
				{
					middlePosition = mSize + mWindowAlignmentOffset
					        - mPaddingLow;
				}
				if (mWindowAlignmentOffsetPercent != WINDOW_ALIGN_OFFSET_PERCENT_DISABLED)
				{
					middlePosition += (int) (mSize
					        * mWindowAlignmentOffsetPercent / 100);
				}
			}
			else
			{
			        ...
			        && (mWindowAlignment & WINDOW_ALIGN_BOTH_EDGE) == WINDOW_ALIGN_BOTH_EDGE)
			{
				...
				{
					// total children size is less than view port and we want to
					// align
			...
			if (!isMinUnknown)
			{
				...
				{
					// scroll center is within half of view port size: align the
					// start edge
			...
			if (!isMaxUnknown)
			{
				...
				{
					// scroll center is very close to the end edge of view port
					// : align the
					// end edge of last children (plus expanded size) to view
					// port's end
		...
		public String toString()
		{
			return "center: " + mScrollCenter + " min:" + mMinEdge + " max:"
			        + mMaxEdge;
		}
