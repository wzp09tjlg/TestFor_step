...
/**
 * A dynamic data structure that caches staggered grid position information for
 * each individual child. The algorithm ensures that each row will be kept as
 * balanced as possible when prepending and appending a child.
 *
 * <p>
 * You may keep view {@link Location} inside StaggeredGrid as much as possible
 * since prepending and appending views is not symmetric: layout going from 0 to
 * N will likely produce a different result than layout going from N to 0 for
 * the staggered cases. If a user scrolls from 0 to N then scrolls back to 0 and
 * we don't keep history location information, edges of the very beginning of
 * rows will not be aligned. It is recommended to keep a list of tens of
 * thousands of {@link Location}s which will be big enough to remember a typical
 * user's scroll history.
 *
 * <p>
...
abstract class StaggeredGrid extends Grid
{
	...
	protected CircularArray<Location> mLocations = new CircularArray<Location>(
	        64);
	...
	@Override
	protected final boolean prependVisibleItems(int toLimit,
	...
	/**
	 * Prepends items using cached locations, returning true if toLimit is
	 * reached. This method should only be called by prependVisibleItems().
	 */
	protected final boolean prependVisbleItemsWithCache(int toLimit,
	        boolean oneColumnMode)
	{
		if (mLocations.size() == 0)
		{
		...
		else
		{
			...
			{
				// if the item is not within or adjacent to cached items, clear
				// cache.
				mLocations.clear();
				return false;
			}
			...
			{
				// if the item is adjacent to first index, should prepend
				// without cache.
				return false;
			}
		}
	...
	private int calculateOffsetAfterLastItem(int row)
	{
		...
		int offset = isReversedFlow() ? -getLocation(cachedIndex).size
		        - mMargin : getLocation(cachedIndex).size + mMargin;
	...
	/**
	 * This implements the algorithm of layout staggered grid, the method should
	 * only be called by prependVisibleItems().
	 */
	protected abstract boolean prependVisibleItemsWithoutCache(int toLimit,
	        boolean oneColumnMode);
	 ...
	 */
	protected final int prependVisibleItemToRow(int itemIndex, int rowIndex,
	        int edge)
	{
		if (mFirstVisibleIndex >= 0)
		{
			if (mFirstVisibleIndex != getFirstIndex()
			        || mFirstVisibleIndex != itemIndex + 1)
			{
				// should never hit this when we prepend a new item with a new
				// Location object.
				throw new IllegalStateException();
			}
		}
		Location oldFirstLoc = mFirstIndex >= 0 ? getLocation(mFirstIndex)
		        : null;
	...
	@Override
	protected final boolean appendVisibleItems(int toLimit,
	        boolean oneColumnMode)
	{
		if (mProvider.getCount() == 0)
		{
	...
	/**
	 * Appends items using cached locations, returning true if at least one item
	 * is appended and (oneColumnMode is true or reach limit and aboveIndex).
	 * This method should only be called by appendVisibleItems()
	 */
	protected final boolean appendVisbleItemsWithCache(int toLimit,
	        boolean oneColumnMode)
	{
		if (mLocations.size() == 0)
		{
	 ...
	 * appendVisibleItems().
	 */
	protected abstract boolean appendVisibleItemsWithoutCache(int toLimit,
	        boolean oneColumnMode);
	 ...
	 */
	protected final int appendVisibleItemToRow(int itemIndex, int rowIndex,
	        int location)
	{
		int offset;
		if (mLastVisibleIndex >= 0)
		{
			if (mLastVisibleIndex != getLastIndex()
			        || mLastVisibleIndex != itemIndex - 1)
			{
				// should never hit this when we append a new item with a new
				// Location object.
				throw new IllegalStateException();
			}
		}
		if (mLastVisibleIndex < 0)
		{
			// if we append first visible item after existing cached items, we
			// need update
	...
	@Override
	public final CircularIntArray[] getItemPositionsInRows(int startPos,
	        int endPos)
	{
	...
	{
		/**
		 * Offset to previous item location. min_edge(index) - min_edge(index -
		 * 1) for non reversed case max_edge(index) - max_edge(index - 1) for
		 * reversed case
		 */
		public int offset;
