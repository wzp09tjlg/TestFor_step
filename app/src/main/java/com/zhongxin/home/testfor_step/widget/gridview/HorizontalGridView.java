...
/**
 * A {@link android.view.ViewGroup} that shows items in a horizontal scrolling
 * list. The items come from the {@link RecyclerView.Adapter} associated with
 * this view.
 * <p>
 * {@link RecyclerView.Adapter} can optionally implement
 * {@link FacetProviderAdapter} which provides {@link FacetProvider} for a given
 * view type; {@link RecyclerView.ViewHolder} can also implement
 * {@link FacetProvider}. Facet from ViewHolder has a higher priority than the
 * one from FacetProiderAdapter associated with viewType. Supported optional
 * facets are:
 * <ol>
 * <li> {@link ItemAlignmentFacet} When this facet is provided by ViewHolder or
 * FacetProviderAdapter, it will override the item alignment settings set on
 * HorizontalGridView. This facet also allows multiple alignment positions
 * within one ViewHolder.</li>
 * </ol>
 */
public class HorizontalGridView extends BaseGridView
{
	...
	protected void initAttributes(Context context, AttributeSet attrs)
	{
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
		        R.styleable.lbHorizontalGridView);
		setRowHeight(a);
	...
	{
		TypedValue typedValue = array
		        .peekValue(R.styleable.lbHorizontalGridView_rowHeight);
		if (typedValue != null)
		{
			int size = array.getLayoutDimension(
			        R.styleable.lbHorizontalGridView_rowHeight, 0);
			setRowHeight(size);
		}
	}
	 ...
	 *            WRAP_CONTENT}, or a size in pixels. If zero, row height will
	 *            be fixed based on number of rows and view height.
	 */
	...
	/**
	 * Sets the fade out left edge to transparent. Note turn on fading edge is
	 * very expensive that you should turn off when HorizontalGridView is
	 * scrolling.
	 */
	...
	public final void setFadingLeftEdgeLength(int fadeLength)
	{
		if (mLowFadeShaderLength != fadeLength)
		{
			...
			{
				mLowFadeShader = new LinearGradient(0, 0, mLowFadeShaderLength,
				        0, Color.TRANSPARENT, Color.BLACK,
	...
	/**
	 * Returns the distance in pixels between fading start position and left
	 * padding edge. The fading start position is positive when start position
	 * is inside left padding area. Default value is 0, means that the fading
	 * starts from left padding edge.
	 */
	...
	/**
	 * Sets the distance in pixels between fading start position and left
	 * padding edge. The fading start position is positive when start position
	 * is inside left padding area. Default value is 0, means that the fading
	 * starts from left padding edge.
	 */
	...
	/**
	 * Sets the fade out right edge to transparent. Note turn on fading edge is
	 * very expensive that you should turn off when HorizontalGridView is
	 * scrolling.
	 */
	...
	public final void setFadingRightEdgeLength(int fadeLength)
	{
		if (mHighFadeShaderLength != fadeLength)
		{
			...
			{
				mHighFadeShader = new LinearGradient(0, 0,
				        mHighFadeShaderLength, 0, Color.BLACK,
	...
	/**
	 * Sets the distance in pixels between fading start position and right
	 * padding edge. The fading start position is positive when start position
	 * is inside right padding area. Default value is 0, means that the fading
	 * starts from right padding edge.
	 */
	...
	/**
	 * Returns the distance in pixels between fading start position and right
	 * padding edge. The fading start position is positive when start position
	 * is inside right padding area. Default value is 0, means that the fading
	 * starts from right padding edge.
	 */
	...
	private boolean needsFadingLowEdge()
	{
		...
		for (int i = 0; i < c; i++)
		{
			...
			if (mLayoutManager.getOpticalLeft(view) < getPaddingLeft()
	...
	private Bitmap getTempBitmapLow()
	{
		...
		{
			mTempBitmapLow = Bitmap.createBitmap(mLowFadeShaderLength,
	...
	private Bitmap getTempBitmapHigh()
	{
		        ...
		        || mTempBitmapHigh.getHeight() != getHeight())
		{
			...
			{
				mTempBitmapHigh = Bitmap.createBitmap(mHighFadeShaderLength,
                        getHeight(), Bitmap.Config.ARGB_8888);
			}
		}
	...
	public void draw(Canvas canvas)
	{
		...
		int lowEdge = mFadingLowEdge ? getPaddingLeft() - mLowFadeShaderOffset
		...
		canvas.clipRect(lowEdge + (mFadingLowEdge ? mLowFadeShaderLength : 0),
		        0, highEdge - (mFadingHighEdge ? mHighFadeShaderLength : 0),
		        getHeight());
		super.draw(canvas);
		...
		if (needsFadingLow && mLowFadeShaderLength > 0)
		{
			...
			tmpCanvas.drawRect(0, 0, mLowFadeShaderLength, getHeight(),
			        mTempPaint);
			// copy back to canvas
			mTempRect.left = 0;
		...
		if (needsFadingHigh && mHighFadeShaderLength > 0)
		{
			...
			tmpCanvas.drawRect(0, 0, mHighFadeShaderLength, getHeight(),
			        mTempPaint);
			// copy back to canvas
			mTempRect.left = 0;
	...
	/**
	 * Updates the layer type for this view. If fading edges are needed, use a
	 * hardware layer. This works around the problem that when a child
	 * invalidates itself (for example has an animated background), the parent
	 * view must also be invalidated to refresh the display list which updates
	 * the the caching bitmaps used to draw the fading edges.
	 */
